...
define('mobiledoc-experiments/ghost-cards/slide-show/image-list', ['exports', 'jquery'], function (exports, _jquery) {
	var _createClass = (function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ('value' in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; })();
	...
	if (!window.__drag) {
		...
		window.addEventListener('mousemove', function (e) {
			return window.__mouseMove(e);
		});
		window.addEventListener('mouseup', function (e) {
			return window.__mouseUp(e);
		});
	}

	var ImageList = (function () {
		function ImageList(imageSlides, width) {
			var _this = this;
			...
			var imagePreviews = this.imagePreviews = []; //preview images
			this.dragImage;
			this.dropImageLocation = null; //the offset location of the current hover location

			var holder = this.holder = document.createElement('div');
			var lines = this.lines = [[]];
			var hoverImage = { image: null, inside: null };
			holder.className = 'card-slideshow-preview';

			imageSlides.forEach(function (image, i) {
				return imagePreviews.push(new ImagePreview(image, i));
			});

			window.__mouseMove = function (e) {
				window.__drag(e);
				if (window.__dragging) {
					imagePreviews.forEach(function (image, i) {
						if (image !== window.__dragging) {
							var inside = image.inside(e.pageX, e.pageY);
							...
							if (inside) {
								if (inside === 'left') {
									_this.dropImageLocation = i;
								} else {
									_this.dropImageLocation = i + 1;
								}
								hoverImage = { image: image, inside: inside };
								_this.rerender();
							}
						}
					});
			...
			window.__mouseUp = function (e) {
				if (window.__dragging) {
					...
					var $holder = (0, _jquery['default'])(_this.holder);
					...
					if (e.pageX > offset.left && e.pageX < offset.left + _width && e.pageY > offset.top && e.pageY < offset.top + height) {
						_this.holder.appendChild(window.__dragging.image);
						_this.imageSlides.splice(window.__dragging.offset, 1);
						_this.imageSlides.splice(_this.dropImageLocation, 0, window.__dragging.slide);
					} else {
						window.__dragging.$image.fadeOut('slow', function () {
							return window.__dragging.$image.remove();
						});
						_this.imageSlides.splice(window.__dragging.offset, 1);
					}
		...
		_createClass(ImageList, [{
			...
			value: function rerender() {
				...
				this.imagePreviews.forEach(function (image, i) {
					if (i === _this2.dropImageLocation) {
						accumulatedWidth += imageWidth;
					}
					if (accumulatedWidth > _this2.width) {
			...
			value: function render() {
				...
				var fade = arguments.length <= 0 || arguments[0] === undefined ? true : arguments[0];
				...
				this.imagePreviews.forEach(function (image, i) {
					...
					if (accumulatedWidth > _this3.width) {
						accumulatedHeight += imageHeight;
						accumulatedWidth = 0;
					}
	...
	var ImagePreview = (function () {
		function ImagePreview(slide, offset) {
			var _this4 = this;
			...
			this.$image = (0, _jquery['default'])(this.image);
			this.dragging = false;
			...
			this.image.addEventListener('mousedown', function (e) {
				var offset = (0, _jquery['default'])(_this4.image).offset();

				dragOffset.left = e.pageX - offset.left;
				dragOffset.top = e.pageY - offset.top;

				document.body.appendChild(_this4.image);
				_this4.image.style.zIndex = '999999';

				_this4.image.style.position = "absolute";
				_this4.image.style.left = e.pageX - dragOffset.left + "px";
				_this4.image.style.top = e.pageY - dragOffset.top + "px";

				window.__drag = mouseMove.bind(_this4);
				window.__drop = mouseUp.bind(_this4);
				window.__dragging = _this4;
		...
		_createClass(ImagePreview, [{
			...
			value: function createImagePreview(path) {
...
define('mobiledoc-experiments/ghost-cards/slide-show/index', ['exports', 'mobiledoc-experiments/ghost-cards/card', 'jquery', 'mobiledoc-experiments/ghost-cards/slide-show/image-slide', 'mobiledoc-experiments/ghost-cards/slide-show/image-list'], function (exports, _mobiledocExperimentsGhostCardsCard, _jquery, _mobiledocExperimentsGhostCardsSlideShowImageSlide, _mobiledocExperimentsGhostCardsSlideShowImageList) {
  var _createClass = (function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ('value' in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; })();
...
define("mobiledoc-experiments/ghost-cards/test", ["exports"], function (exports) {
  console.log("TEST");
});
